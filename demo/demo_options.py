...
class DemoOptions:
    def __init__(self):
        ...
        default_checkpoint_body_smpl = (
            "./extra_data/body_module/pretrained_weights/2020_05_31-00_50_43-best-51.749683916568756.pt"
        )
        parser.add_argument(
        ...
        parser.add_argument(
            "--checkpoint_hand", required=False, default=default_checkpoint_hand, help="Path to pretrained checkpoint"
        )
        ...
        parser.add_argument(
            "--start_frame", type=int, default=0, help="given a sequence of frames, set the starting frame"
        )
        parser.add_argument(
            "--end_frame", type=int, default=float("inf"), help="given a sequence of frames, set the last frame"
        )
        parser.add_argument("--pkl_dir", type=str, help="Path of storing pkl files that store the predicted results")
        parser.add_argument(
            "--openpose_dir", type=str, help="Directory of storing the prediction of openpose prediction"
        )
        ...
        parser.add_argument(
            "--save_bbox_output", action="store_true", help="Save the bboxes in json files (bbox_xywh format)"
        )
        parser.add_argument(
            "--save_pred_pkl", action="store_true", help="Save the predictions (bboxes, params, meshes in pkl format"
        )
        parser.add_argument("--save_mesh", action="store_true", help="Save the predicted vertices and faces")
        parser.add_argument(
            "--save_frame", action="store_true", help="Save the extracted frames from video input or webcam"
        )
        ...
        parser.add_argument(
            "--no_video_out", action="store_true", help="Do not merge rendered frames to video (ffmpeg)"
        )
        parser.add_argument(
            "--smpl_dir", type=str, default="./extra_data/smpl/", help="Folder where smpl files are located."
        )
        ...
        # Mesh Graphormer specific options
        parser.add_argument(
            "--num_hidden_layers", default=4, type=int, required=False, help="Update model config if given"
        )
